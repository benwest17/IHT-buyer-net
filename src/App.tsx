...
function calcBuyerFees(s: BuyerFeeSettings): { items: BuyerFeeItem[]; total: number } {
  ...
  const closingProcessingFee = isValpo ? FEES.closingProcessingValpo : FEES.closingProcessing;
  const titleProcessingFee = isValpo ? FEES.titleProcessingValpo : FEES.titleProcessing;

  if (s.includeClosingFee) items.push({ label: "Closing fee", amount: FEES.closingFee });
  ...
  if (s.includeCPLBorrower) items.push({ label: "CPL (borrower)", amount: FEES.cplBorrower });
  if (s.includeCPLLender) items.push({ label: "CPL (lender)", amount: FEES.cplLender });
  if (s.includeDocumentPrep) items.push({ label: "Document prep", amount: FEES.documentPrep });
  if (s.includeLenderPolicy) items.push({ label: "Lender's title policy", amount: FEES.lenderPolicy });
  const count = Math.max(0, Math.min(4, Math.floor(s.endorsementCount || 0)));
  if (count > 0) items.push({ label: `Endorsements (${count} @ $50)`, amount: count * FEES.endorsementPer });
  if (s.includeSimplifile) items.push({ label: "Simplifile e-recording", amount: FEES.simplifile });
  if (s.includeTIEFF) items.push({ label: "TIEFF", amount: FEES.tieff });
  if (s.includeRecordingFees) items.push({ label: "Recording fees", amount: FEES.recordingFees });
  if (s.includeSalesDisclosureTransfer) items.push({ label: "Sales disclosure / transfer", amount: FEES.salesDisclosureTransfer });
...
function buildPdf(opts: {
  ...
  const rows: Array<[string, string]> = [
    ...
    ["Loan amount", toMoney(opts.loanAmount)],
    ["Down payment", toMoney(opts.downPayment)],
...
export default function App() {
  ...
  const [includeDocumentPrep, setIncludeDocumentPrep] = useState(true);
  ...
  const loanAmount = useMemo(() => parseNumber(loanInput), [loanInput]);
  const downPayment = useMemo(() => parseNumber(downPaymentInput), [downPaymentInput]);
            ...
            <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Field label="Loan amount">
                  <input value={loanInput} onChange={(e) => setLoanInput(formatInputMoney(e.target.value))} className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none focus:border-neutral-900" inputMode="decimal" />
                </Field>
                <Field label="Down payment">
                  <input value={downPaymentInput} onChange={(e) => setDownPaymentInput(formatInputMoney(e.target.value))} className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none focus:border-neutral-900" inputMode="decimal" />
                </Field>
              ...
              <div className="mt-8 rounded-3xl bg-neutral-50 p-5 ring-1 ring-black/5">
                ...
                <p className="mt-1 text-sm text-neutral-600">Generic endorsements are $50 each. Default is 2 endorsements, max is 4.</p>
                <div className="mt-2 text-xs text-neutral-500">
                  {isValpo ? "Using Valpo county override: Closing Processing = $175, Title Processing = $225" : "Using standard county pricing"}
                </div>
                <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <div className="grid grid-cols-1 gap-2">
                      <CheckRow label="Closing fee" checked={includeClosingFee} onChange={setIncludeClosingFee} />
                      <CheckRow label={`Closing processing fee (${toMoney(isValpo ? FEES.closingProcessingValpo : FEES.closingProcessing)})`} checked={includeClosingProcessing} onChange={setIncludeClosingProcessing} />
                      <CheckRow label={`Title processing fee (${toMoney(isValpo ? FEES.titleProcessingValpo : FEES.titleProcessing)})`} checked={includeTitleProcessing} onChange={setIncludeTitleProcessing} />
                      <CheckRow label="CPL (borrower)" checked={includeCPLBorrower} onChange={setIncludeCPLBorrower} />
                      <CheckRow label="CPL (lender)" checked={includeCPLLender} onChange={setIncludeCPLLender} />
                      <CheckRow label="Document prep" checked={includeDocumentPrep} onChange={setIncludeDocumentPrep} />
                    </div>
                  </div>
                  <div>
                    <div className="grid grid-cols-1 gap-2">
                      <CheckRow label="Lender's title policy" checked={includeLenderPolicy} onChange={setIncludeLenderPolicy} />
                      <CheckRow label="Simplifile e-recording" checked={includeSimplifile} onChange={setIncludeSimplifile} />
                      <CheckRow label="TIEFF" checked={includeTIEFF} onChange={setIncludeTIEFF} />
                      <CheckRow label="Recording fees" checked={includeRecordingFees} onChange={setIncludeRecordingFees} />
                      ...
                      <Field label="Endorsement count" hint="Default 2, max 4">
          ...
          <motion.div {...cardMotion} className="lg:col-span-1">
            <div className="sticky top-6 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
              ...
              <div className="mt-4 space-y-3">
                ...
                <Row k="Loan amount" v={toMoney(round2(loanAmount))} />
                <Row k="Down payment" v={toMoney(round2(downPayment))} />
