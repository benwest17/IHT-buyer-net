...
function calcBuyerFees(s: BuyerFeeSettings): { items: BuyerFeeItem[]; total: number } {
  ...
  if (s.transactionType === "mortgage") {
    ...
    if (s.includeDocumentPrep) items.push({ label: "Document prep", amount: MORTGAGE_FEES.documentPrep });
    ...
    const titleProcessingFee = isValpo ? CASH_FEES.titleProcessingValpo : CASH_FEES.titleProcessing;
    ...
    if (s.includeDocumentPrep) items.push({ label: "Document prep", amount: CASH_FEES.documentPrep });
...
function buildPdf(opts: {
  ...
  const rows: Array<[string, string]> = [
    ...
    ["Down payment / funds brought", toMoney(opts.downPayment)],
...
export default function App() {
  ...
  const [showEndorsementInfo, setShowEndorsementInfo] = useState(false);
  ...
  const buyerFeeCalc = useMemo(() => calcBuyerFees({
    ...
    includeCPLBorrower, includeCPLLender, includeDocumentPrep, includeLenderPolicy, includeSimplifile,
          ...
          <motion.div {...cardMotion} className="lg:col-span-2">
            <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Field label={transactionType === "mortgage" ? "Down payment" : "Funds brought to closing"}>
              ...
              <div className="mt-8 rounded-3xl bg-neutral-50 p-5 ring-1 ring-black/5">
                ...
                <div className="mt-2 text-xs text-neutral-500">
                  {isValpo ? "Valpo override active: Closing Processing = $175" + (transactionType === "mortgage" ? ", Title Processing = $225" : ", Title Processing = $225") : "Using standard county pricing"}
                </div>
                <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <div className="grid grid-cols-1 gap-2">
                      ...
                      <CheckRow label={`Title processing fee (${toMoney(isValpo ? (transactionType === "mortgage" ? MORTGAGE_FEES.titleProcessingValpo : CASH_FEES.titleProcessingValpo) : (transactionType === "mortgage" ? MORTGAGE_FEES.titleProcessing : CASH_FEES.titleProcessing))})`} checked={includeTitleProcessing} onChange={setIncludeTitleProcessing} />
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      ...
                      {transactionType === "mortgage" && (
                        <Field
                          label={
                            <span className="inline-flex items-center gap-2">
                              ...
                              <button
                                type="button"
                                onClick={() => setShowEndorsementInfo((v) => !v)}
                                className="rounded-full p-1 text-neutral-600 hover:bg-neutral-100"
                                aria-label="Show endorsement info"
                              >
                                <Info size={14} />
                              </button>
                            </span> as any
                        ...
                        >
                          ...
                          {showEndorsementInfo && (
                            <div className="mt-2 rounded-2xl bg-white p-3 text-xs text-neutral-600 ring-1 ring-black/5">
          ...
          <motion.div {...cardMotion} className="lg:col-span-1">
            <div className="sticky top-6 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
              ...
              <div className="mt-4 space-y-3">
                ...
                <Row k={transactionType === "mortgage" ? "Down payment" : "Funds brought"} v={toMoney(round2(downPayment))} />
