...
export default function App() {
  ...
  useEffect(() => {
    const target = purchasePriceInput || "0";
    if (transactionType === "cash" && downPaymentInput !== target) {
      setDownPaymentInput(target);
    }
  }, [transactionType, purchasePriceInput, downPaymentInput]);
            ...
            <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <Field label={transactionType === "mortgage" ? "Down payment" : "Funds needed to close"}>
                  <input value={downPaymentInput} onChange={(e) => setDownPaymentInput(formatInputMoney(e.target.value))} className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none focus:border-neutral-900" inputMode="decimal" disabled={transactionType === "cash"} />
                </Field>
